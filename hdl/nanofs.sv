  ...
  always_comb begin
    ...
    case (current_state)
      IDLE: begin
        ...
        spi_rst = 1'b1;
        if (start == 1'b1) next_state = RESET_SPI;
      end
      RESET_SPI: begin
      ...
      INIT_NANOFS: begin
        ...
        if (init_nanofs_success) next_state = PRE_SEARCH_NANOFS;
        else if (init_nanofs_err) next_state = ERROR;

      end
      PRE_SEARCH_NANOFS: begin
      ...
      SEARCH_NANOFS: begin
        ...
        if (search_nanofs_success) next_state = PRE_EXECUTE_NANOFS;
        else if (search_nanofs_err) next_state = NOT_FILE;

      end
      PRE_EXECUTE_NANOFS: begin

        sel_mux = 2'b11;
        next_state = EXECUTE_NANOFS;
      end
      EXECUTE_NANOFS: begin

        sel_mux = 2'b11;

        execute_nanofs_start = 1'b1;
        if (end_of_file) next_state = END_FSM;
      end
      END_FSM: begin
        finish_signal = 1'b1;
